...
output: MP4またはMKV、通常ASSまたは装飾ASS、互換用SRT、edit_plan.json、プロジェクトJSON
...
STEP_TRANSCRIBE | SCREEN_TRANSCRIBE | 字幕候補と発話解析結果の生成 | 有効なプロジェクト | transcriptと字幕候補が有効 | STEP_AI_SUBTITLE以降
...
STEP_SUBTITLE_EDIT | SCREEN_SUBTITLE_EDIT | カット後タイムラインの字幕本文、発話時刻、ASS個別スタイルを確定 | カット済みedit_planが存在 | edited subtitlesが保存済み | STEP_DECORATION以降
...
- `subtitles/*.srt`: 文字起こしと編集互換用の中間成果物。スタイルの正本にはしない。
- 通常ASS: `project.json` のASS既定値と字幕ごとの `ass_style` から生成する。
- 装飾ASS: 通常字幕情報に `decoration_project.json` の装飾を加えて生成する。
