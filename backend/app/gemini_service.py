...
def translate_project_subtitles(
    project_id: str,
    model: str | None = None,
    ...
    display_mode = display_mode if display_mode in DISPLAY_MODES else "source_above"
    ...
    edit_plan = load_project_edit_plan(project_id) if edit_plan_path.exists() else None
    subtitles = list((edit_plan or {}).get("subtitles") or transcript.get("subtitles") or [])
    source_items = [
        {"id": str(item.get("id") or f"sub_{index + 1:04d}"), "text": str(item.get("source_text") or item.get("text") or "").strip()}
        for index, item in enumerate(subtitles)
        if item.get("enabled", True) is not False and str(item.get("source_text") or item.get("text") or "").strip()
    ]
    if not source_items:
        raise HTTPException(status_code=400, detail="翻訳する字幕がありません")
    try:
        from google import genai
    except ImportError as exc:
        raise HTTPException(status_code=500, detail="google-genaiが未導入です。setup.batの標準構成を実行してください") from exc

    client = genai.Client(api_key=_api_key())
    translated_by_id: dict[str, str] = {}
    try:
        for offset in range(0, len(source_items), 60):
            batch = source_items[offset : offset + 60]
            prompt = f"""字幕を{source_language}から{target_language}へ自然に翻訳してください。
入力のidは変更・省略・追加せず、同じidで返してください。時刻情報は変更しません。
字幕として簡潔で読みやすい日本語を優先し、固有名詞と文脈を保ってください。
原文にない説明、話者名、注釈を追加しないでください。
...
{json.dumps(batch, ensure_ascii=False)}"""
            interaction = client.interactions.create(
                ...
                input=prompt,
                response_format={"type": "text", "mime_type": "application/json", "schema": _translation_schema()},
            )
            ...
            for item in result.get("translations") or []:
                ...
                if item_id and translated:
                    translated_by_id[item_id] = translated
    except Exception as exc:
        audit_event("gemini.translation.failed", project_id=project_id, context={"model": selected_model, "error_type": type(exc).__name__})
        raise HTTPException(status_code=502, detail=f"Gemini字幕翻訳に失敗しました: {exc}") from exc

    updated = []
    for index, item in enumerate(subtitles):
        next_item = dict(item)
        item_id = str(next_item.get("id") or f"sub_{index + 1:04d}")
        source_text = str(next_item.get("source_text") or next_item.get("text") or "").strip()
        next_item["id"] = item_id
        next_item["source_text"] = source_text
        if item_id in translated_by_id:
            next_item["translated_text"] = translated_by_id[item_id]
        next_item["source_language"] = source_language
        next_item["target_language"] = target_language
        next_item["subtitle_display_mode"] = display_mode
        next_item["bilingual_enabled"] = True
        updated.append(next_item)

    transcript = dict(transcript)
    transcript["subtitles"] = updated
    transcript["bilingual_translation"] = {
        "model": selected_model,
    ...
    if edit_plan is not None:
        edit_plan["subtitles"] = updated
        atomic_write_json(edit_plan_path, edit_plan, backup=True)
    write_srt(updated, base / "subtitles" / "edited.srt")
    audit_event("gemini.translation.completed", project_id=project_id, context={"model": selected_model, "translated_count": len(translated_by_id)})
    return {
        "model": selected_model,
        "translated_count": len(translated_by_id),
        "subtitles": updated,
        ...
        "edit_plan": edit_plan,
        "srt_path": str(base / "subtitles" / "edited.srt"),
    }
...
def _transcript_schema(include_speaker: bool = True, bilingual: bool = False) -> dict[str, Any]:
    segment_properties = {
        "start_sec": {"type": "number"},
        "end_sec": {"type": "number"},
    }
    if bilingual:
        segment_properties["source_text"] = {"type": "string"}
        segment_properties["translated_text"] = {"type": "string"}
        segment_required = ["start_sec", "end_sec", "source_text", "translated_text"]
    else:
        segment_properties["text"] = {"type": "string"}
...
def transcribe_project_with_gemini(
    project_id: str,
    model: str | None = None,
    language: str = "ja",
    ...
    speaker_labels_enabled = bool(config.get("speaker_labels_enabled", DEFAULT_SPEAKER_LABELS_ENABLED))
    srt_timing_priority = bool(config.get("srt_timing_priority", DEFAULT_SRT_TIMING_PRIORITY))
    bilingual_settings = normalize_bilingual_settings(bilingual_subtitle_settings)
    bilingual_enabled = bool(
        bilingual_settings["enabled"]
        and bilingual_settings["source_language"] != bilingual_settings["target_language"]
    )
    try:
        from google import genai
    except ImportError as exc:
...
短い相づちも独立して聞こえる場合は省略せず、長すぎる本文は読みやすい文節で分けてください。"""
    if bilingual_enabled:
        language_instruction = f"""音声を{bilingual_settings['source_language']}の原文として正確に文字起こしし、同じsegmentごとに{bilingual_settings['target_language']}へ翻訳してください。
source_textには実際に聞こえた原文だけを記録し、翻訳や要約を混ぜないでください。
translated_textにはsource_textの自然で簡潔な翻訳だけを記録してください。
原文と翻訳のsegment数・start_sec・end_secは必ず共通にしてください。"""
    else:
        language_instruction = f"音声を{language or 'ja'}の字幕として正確に文字起こししてください。"
...
字幕本文は読みやすい句読点を付け、同じ発話を細かく分割しすぎないでください。"""
    ...
    try:
        ...
        interaction = client.interactions.create(
            ...
            response_format={"type": "text", "mime_type": "application/json", "schema": _transcript_schema(speaker_labels_enabled, bilingual_enabled)},
        )
        result = json.loads(interaction.output_text)
    except Exception as exc:
    ...
    for index, segment in enumerate(result.get("segments") or [], start=1):
        ...
        translated_text = str(segment.get("translated_text") or "").strip() if bilingual_enabled else ""
        if not speaker_labels_enabled:
            source_text = _strip_generated_speaker_prefix(source_text)
            translated_text = _strip_generated_speaker_prefix(translated_text)
        if not source_text:
            continue
        if bilingual_enabled and not translated_text:
            raise HTTPException(status_code=502, detail="Geminiから日本語訳が返りませんでした。二言語字幕を再実行してください")
        ...
        subtitles.append({
            ...
            "source_language": bilingual_settings["source_language"] if bilingual_enabled else language,
            "target_language": bilingual_settings["target_language"] if bilingual_enabled else "",
            "subtitle_display_mode": bilingual_settings["display_mode"] if bilingual_enabled else "source_only",
    ...
    transcript = {
        ...
        "bilingual_subtitle_settings": bilingual_settings,
        "subtitles": subtitles,
