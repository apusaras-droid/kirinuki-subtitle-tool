...
初回セットアップをまとめて実行する場合は `download-runtime.bat` を使います。
...
- `download-runtime.bat` : Python依存関係と既定モデルを取得
- `download-runtime.bat skip-pip` : モデルだけ取得
- `download-runtime.bat models-only` : `skip-pip` と同じ

取得対象:

- Python依存関係
- `tools/whisper.cpp/models/ggml-large-v3.bin`
- `tools/whisper.cpp/models/ggml-silero-v5.1.2.bin`
...
初回導線を1本にまとめる場合は `setup.bat` を使います。

```bat
setup.bat
```

これは `download-runtime.bat` と `download-ffmpeg.bat` を順番に呼びます。
完了後に `launch.bat` を起動します。
...
`LICENSE` と `licenses/` の雛形は配布用に埋める前提のものです。
