...
- `4 モデルのみ`: Base / Small / Medium / Large-v3 / 全モデルから選んで取得またはハッシュ再検証
...
- Python依存関係
- `tools/whisper.cpp/models/ggml-small.bin`（アプリ既定）
- `tools/whisper.cpp/models/ggml-silero-v6.2.0.bin`

フル構成では `ggml-large-v3.bin` も取得します。別のサイズは `setup.bat` の `4 モデルのみ` から追加できます。
...
- 既定モデル: `tools/whisper.cpp/models/ggml-small.bin`
- UIのモデル欄では導入済みの `base`、`small`、`medium`、`large-v3` またはモデルファイルのフルパスを指定できます。
